...
{opp.kalshi_market.platform.title()}: Bet ${opp.bet_kalshi_amount:.2f} on {opp.bet_kalshi_side.upper()}
...
{opp.kalshi_market.platform.title()}:
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="bet">
                <strong>{opp.kalshi_market.platform.title()}:</strong> Bet <strong>${opp.bet_kalshi_amount:.2f}</strong> on <strong>{opp.bet_kalshi_side.upper()}</strong>
            </div>
        ...
        <div class="section">
            ...
            <h3>{opp.kalshi_market.platform.title()}</h3>
            <p>YES: {opp.kalshi_market.yes_price * 100:.2f}% | NO: {opp.kalshi_market.no_price * 100:.2f}%</p>
            <a href="{opp.kalshi_market.url}" class="link">View on {opp.kalshi_market.platform.title()}</a>
        </div>
    </div>
</body>
</html>
    ...
    def _format_text_batch_alert(self, opportunities: List[ArbitrageOpportunity]) -> str:
        ...
        for i, opp in enumerate(opportunities, 1):
            ...
            text += f"${opp.bet_kalshi_amount:.2f} on {opp.bet_kalshi_side.upper()} ({opp.kalshi_market.platform.title()})\n"
        ...
        for i, opp in enumerate(opportunities, 1):
            ...
            <div class="opp-item">
                ...
                <p>Strategy: ${opp.bet_polymarket_amount:.2f} on {opp.bet_polymarket_side.upper()} (Polymarket), 
                   ${opp.bet_kalshi_amount:.2f} on {opp.bet_kalshi_side.upper()} ({opp.kalshi_market.platform.title()})</p>
                <p>
                    <a href="{opp.polymarket_market.url}" class="link">Polymarket</a>
                    <a href="{opp.kalshi_market.url}" class="link">{opp.kalshi_market.platform.title()}</a>
                </p>
            </div>
            """
...
• {opportunity.kalshi_market.platform.title()}: ${opportunity.bet_kalshi_amount:.2f} on {opportunity.bet_kalshi_side.upper()}

*Returns:*
• Min Profit: ${opportunity.min_profit:.2f}
• ROI: {opportunity.roi_percent:.2f}%

[Polymarket]({opportunity.polymarket_market.url}) | [{opportunity.kalshi_market.platform.title()}]({opportunity.kalshi_market.url})
            """
